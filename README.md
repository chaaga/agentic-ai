...
└── rfp-assistant-excel/
    ...
    ├── simple.py             # minimal RAG demo (no reranking)
...
1. Place your knowledge base in `rfp-assistant-excel/data/repository.xlsx`
   with `Question` and `Answer` columns.
2. Place new RFP questions in `rfp-assistant-excel/data/new_questions.xlsx`
   with a `Question` column.
3. Run the assistant:

```
cd rfp-assistant-excel
...
- **`simple.py`** / **`rag_with_rerank.py`** — minimal, standalone demos of
  the RAG pipeline (used for tech-talk demos).
